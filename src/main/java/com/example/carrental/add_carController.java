...
public class add_carController {
    ...
    public void initialize() {
        ...
        typeField.textProperty().addListener((obs, oldVal, newVal) -> updatePriceSuggestion(newVal));
    }

    private void updatePriceSuggestion(String type) {
        if (type == null || type.trim().isEmpty()) {
            suggestedPriceLabel.setText("");
            return;
        }

        PricingAdvisor.PriceSuggestion suggestion = PricingAdvisor.suggestPrice(type.trim());

        if (suggestion.suggestedPrice > 0) {
            suggestedPriceLabel.setText(
                    String.format("Suggested price: %.0f PKR/day — %s",
                            suggestion.suggestedPrice, suggestion.reason)
            );
        } else {
            suggestedPriceLabel.setText(suggestion.reason);
        }
    }
